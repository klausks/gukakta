...
- When creating new pages, follow the archetypes provided in the `archetypes` folder. For example, if you are creating a new page for a location, use the `locations.md` archetype as a template. You may add other sections to the page as needed, but do not remove any sections that are already in the archetype.
- Create links between pages when there are references to other characters, locations or items. For example, if a session note mentions a character named "Strahd", create a page for Strahd if it doesn't exist and link to it from the page where he's mentioned.
