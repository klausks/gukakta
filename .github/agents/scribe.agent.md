...
- When there's mention of another character, location or any other entity, create a link to the corresponding page if it exists, or create a new page if it doesn't. But don't repeat links to the same entity in the same page. Only place a link when the entity is referenced for the first time in the page. For example, if a character is mentioned multiple times in a session summary, only link to their page the first time they are mentioned.